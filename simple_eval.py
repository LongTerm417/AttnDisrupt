...
tf.flags.DEFINE_integer('mask_percent', 80, 'cam mask percent for look ahead')
...
tf.flags.DEFINE_string('ahead', "attn", "lookahead method")
...
if __name__ == '__main__':
    ...
    if 'ens' in FLAGS.attack_method:
        ...
        input_dir = os.path.join(FLAGS.output_dir,
                                 'single_attack',
                                 f'model_{FLAGS.model_name}_method_{FLAGS.attack_method}_gus_{FLAGS.gus_mean}_{FLAGS.gus_std}_patch_{FLAGS.patch_size}_sigma_{FLAGS.sigma}_portion_{FLAGS.portion}_mask_{FLAGS.mask_percent}_ahead_{FLAGS.ahead}')
        result_file = os.path.join(FLAGS.output_dir, 'single_attack')
    ...
    with tf.Graph().as_default():
        ...
        with tf.Session(config=tf.ConfigProto(allow_soft_placement=True)) as sess:
            ...
            for filenames, images in tqdm(load_images(input_dir, batch_shape),
                                          desc=f"Evaluate {FLAGS.model_name}",
                                          ncols=100):
            ...
            save_file = os.path.join(result_file,
                                     f'results_gus_{FLAGS.gus_mean}_{FLAGS.gus_std}_patch_{FLAGS.patch_size}_sigma_{FLAGS.sigma}_portion_{FLAGS.portion}_mask_{FLAGS.mask_percent}_ahead_{FLAGS.ahead}.json')
            result = read_json_file(save_file)
            if FLAGS.attack_method not in result:
                result[FLAGS.attack_method] = dict()
            if FLAGS.model_name not in result[FLAGS.attack_method]:
                result[FLAGS.attack_method][FLAGS.model_name] = dict()
            for i in range(len(model_name)):
                print("Attack Success Rate for {0} : {1:.1f}%".format(model_name[i], success_count[i] / 1000. * 100))
                result[FLAGS.attack_method][FLAGS.model_name][f'{model_name[i]}'] = "{0:.1f}%".format(success_count[i] / 1000. * 100)
            save_json_file(result, save_file)
