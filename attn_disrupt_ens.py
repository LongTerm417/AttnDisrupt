...
tf.flags.DEFINE_integer('patch_size', 55, 'image patch size')
...
tf.flags.DEFINE_string('model_name', "inception_resnet_v2", "Name of the model")

tf.flags.DEFINE_string('attack_method', "ens", "Name of the model")

tf.flags.DEFINE_integer('sigma', 9, "Name of the model")
...
def ensemble_cam(x, y):
    num_classes = 1001
    result = None
    models_names = ['inception_v3', 'inception_v4', 'inception_resnet_v2', 'resnet_v2']
    for model_name in models_names:
        logits_v3, end_points = get_model_results(x=x, model_name=model_name, num_classes=num_classes)
        cam_value = grad_cam(end_points=end_points, predicted_class=y, nb_classes=num_classes)
        if result is None:
            result = cam_value
            continue
        result += cam_value
...
def graph(x, y, i, x_max, x_min, grad):
    ...
    cam_value = ensemble_cam(x, y)

    atten_aug = attention_aug(x, cam_value)
    x_batch = tf.concat([atten_aug, atten_aug / 2., atten_aug / 4., atten_aug / 8., atten_aug / 16.], axis=0)
    '''input diversity'''
    x_input = x_batch
    if 'di' in FLAGS.attack_method.lower():
        x_input = input_diversity(x_batch)
    logits, aux_logits = None, None
    for method in ['inception_v3', 'inception_v4', 'inception_resnet_v2', 'resnet_v2']:
        logits_v3, end_point = get_model_results(x=x_input, model_name=method, num_classes=num_classes)
        if logits is None:
            logits = logits_v3
            aux_logits = end_point['AuxLogits']
            continue
        logits += logits_v3
        if method != 'resnet_v2':
            aux_logits += end_point['AuxLogits']
    ...
    cross_entropy += tf.losses.softmax_cross_entropy(one_hot, aux_logits, label_smoothing=0.0, weights=0.4)

    noise = tf.reduce_mean(
        tf.split(tf.gradients(cross_entropy, x_batch, colocate_gradients_with_ops=True)[0], 5) * tf.constant(
            [1, 1 / 2., 1 / 4., 1 / 8., 1 / 16.])[:, None, None, None, None], axis=0)
...
def get_prediction(x_input, model_name=FLAGS.model_name):
    logits_v3, end_points_v3 = get_model_results(x=x_input, model_name=model_name, num_classes=1001)
    # pred = tf.argmax(end_points_v3['Predictions'], 1)
    return end_points_v3['Predictions']
...
def main(_):
    ...
    with tf.Graph().as_default():
        ...
        models = ['inception_v3', 'inception_v4', 'inception_resnet_v2', 'resnet_v2']
        preds = None
        for model in models:
            if preds is None:
                preds = get_prediction(x_input=x_input, model_name=model)
            else:
                preds += get_prediction(x_input=x_input, model_name=model)
        ...
        # print(f'xxx={x_input, preds, i, x_max, x_min, grad}')
        x_adv, _, _, _, _, _ = tf.while_loop(stop, graph, [x_input, preds, i, x_max, x_min, grad])
